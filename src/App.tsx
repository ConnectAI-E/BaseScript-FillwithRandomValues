  ...
  return <div>
    ...
    <Spin size="large" spinning={loading}>
      <Form wrapperCol={{ span: 17 }}
        ...
        <Form.Select style={{ width: '100%' }} label='选择模式' field="type"
          onChange={(v) => {
            setType(v as any);
          ...
          <Form.Select.Option value={'number'}>数字模式</Form.Select.Option>
          <Form.Select.Option value={'charts'}>密码模式</Form.Select.Option>
        </Form.Select>
        ...
        {<div style={{ display: type === 'charts' ? 'block' : 'none' }}>
          <Form.InputNumber max={100} min={4} style={{ width: '100%' }} type="Number" label={t('密码长度')} field="maxLength">
          </Form.InputNumber>
          {/* 
          ...
          <Form.TextArea
            ...
            label={{
              text: '随机字符',
              extra: <Tooltip content={<p style={{ whiteSpace: 'pre-wrap' }}>{`随机密码将从字符中随机抽取生成。可修改字符内容，以定制密码内容的范围，例如不希望出现特殊字符，可删掉所有特殊字符。`}</p>}><IconHelpCircle style={{ color: 'var(--semi-color-text-2)' }} /></Tooltip>
            }}
            field="chartsRange">
